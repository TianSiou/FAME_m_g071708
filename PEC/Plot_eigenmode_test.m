...
for idx = 1 : size(col, 1)
    ...
    Eigenvector = ele_field(:, col(idx));
%             Vector = abs(Eigenvector(1 : s));
            Vector = Eigenvector(1 : s);
...
%     Vector = abs(Eigenvector(1 : n1   *(n2-1)*(n3-1)) + Eigenvector(n1   *(n2-1)*(n3-1) + 1 : n1   *(n2-1)*(n3-1) + (n1-1)* n2   *(n3-1)) + Eigenvector(n1   *(n2-1)*(n3-1) + (n1-1)* n2   *(n3-1) + 1 : Nc));
