...
function [ew_order, EV, init_Vec, ProjProbDim, stop_flag] = NullSp_GEP_Herm_JDSIRA_Locking_v2_test(A, B, EV, ew_order, ...
...
%               rsdl_vec_tmp = rsdl_vec_tmp - ritz_ew(1) * MtxVecMult_B(ev_crnt_tmp);
%               rsdl_tmp     = norm(rsdl_vec_tmp);
...
%               rsdl_vec_tmp = rsdl_vec_tmp - ev_crnt_tmp * diag(ritz_ew(idx_ew(1:n_zero)));
              ...
              for p = 1 : n_zero        %%%modify by siu
                  ...
                  rsdl_vec_tmp(:,p) = rsdl_vec_tmp(:,p) - MtxVecMult_B(ev_crnt_tmp(:,p)) * ritz_ew(p);
              end
...
%
                ...
                switch CorEq
                    ...
                    case 'SIRA' 
                        ...
                        linear_system_tol = 1.0e-2; %%1.0e-3
...
%                     mtx_A_prod_V                = MtxVecMult_A( mtxV(:,1:iteno+ii) ); 
                    ...
                    mtx_A_prod_V            = MtxVecMult_B( mtxV(:,iteno+ii) );
                    mtxL(1:iteno+ii,iteno+ii) = mtxV(:,1:iteno+ii)' * mtx_A_prod_V;
                    mtxL(iteno+ii,1:iteno+ii-1)   = mtxL(1:iteno+ii-1,iteno+ii)';
